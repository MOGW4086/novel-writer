...
async def index(request: Request, db_mod=Depends(get_db)):
    """
    小説一覧ページ。シリーズ一覧とスタンドアロン小説を表示する。
    スタンドアロン小説は「フィードバック待ち」と「未読・読書中」に分けて渡す。
    """
    series_list = db_mod.get_series_list()
    all_standalone = [_enrich_novel(n) for n in db_mod.get_standalone_novels(limit=100)]
...
async def series_detail(request: Request, series_id: int, db_mod=Depends(get_db)):
    """
    シリーズ詳細ページ。エピソード一覧と読書進捗を表示する。
    """
    series = db_mod.get_series(series_id)
    if series is None:
        raise HTTPException(status_code=404, detail="シリーズが見つかりません")

    novels = [_enrich_novel(n) for n in db_mod.get_novels_by_series(series_id)]
...
async def novel_detail(request: Request, novel_id: int, db_mod=Depends(get_db)):
    """
    小説閲覧ページ。本文・フィードバックフォーム・読書進捗を表示する。
    初回アクセス時は読書進捗レコードを作成する（scroll_percent=0）。
    """
    novel = db_mod.get_novel(novel_id)
    if novel is None:
        raise HTTPException(status_code=404, detail="小説が見つかりません")

    # 初回アクセス時は読書進捗を作成（opened_at を記録）
    progress = db_mod.get_reading_progress(novel_id)
    if progress is None:
        db_mod.upsert_reading_progress(novel_id, scroll_percent=0)
    ...
    if novel.get("series_id"):
        series = db_mod.get_series(novel["series_id"])
        series_novels = [_enrich_novel(n) for n in db_mod.get_novels_by_series(novel["series_id"])]

    # 既存フィードバック
    feedbacks = db_mod.get_feedback(novel_id)
...
async def update_progress(novel_id: int, body: ProgressRequest, db_mod=Depends(get_db)):
    """
    ...
    """
    if db_mod.get_novel(novel_id) is None:
        raise HTTPException(status_code=404, detail="小説が見つかりません")

    is_completed = body.scroll_percent >= 95
    db_mod.upsert_reading_progress(novel_id, body.scroll_percent, is_completed)
...
def _run_knowledge_extraction(comment: str, novel_id: int, db_mod: types.ModuleType) -> None:
    """バックグラウンドタスク: 知見抽出を実行し、結果をDBに記録する。失敗が続いたらLINE通知。"""
    try:
        knowledge.extract_and_save_knowledge(comment, novel_id=novel_id)
        db_mod.save_extraction_log(novel_id, "success")
    except Exception as exc:
        error_type = type(exc).__name__
        error_message = str(exc)
        logger.warning("知見抽出に失敗しました（novel_id=%d）: %s", novel_id, exc, exc_info=True)
        db_mod.save_extraction_log(novel_id, "failure", error_type=error_type, error_message=error_message)

        consecutive = db_mod.get_consecutive_failure_count()
...
async def submit_feedback(
    ...
    db_mod.save_feedback(novel_id, rating, clean_comment)

    if clean_comment:
        background_tasks.add_task(_run_knowledge_extraction, clean_comment, novel_id, db_mod)
