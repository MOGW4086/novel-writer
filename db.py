...
# DBファイルのパス
DB_PATH = Path(__file__).parent / "data" / "novels.db"
    ...
    with get_connection() as conn:
        for genre in config["genres"]:
            existing = conn.execute(
                "SELECT id FROM genre_settings WHERE name = ?", (genre["name"],)
            ).fetchone()
            if existing:
                continue
            conn.execute(
                """
                INSERT INTO genre_settings (name, description, weight, sub_themes, active)
                VALUES (?, ?, ?, ?, ?)
                """,
                (
                    genre["name"],
